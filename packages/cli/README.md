...
The JSONL report is append-only. Each run appends a `run-start` row with a new run ID, one `group-complete` or `group-unavailable` row per inspected `TopicAssoc` group, and a `run-complete` row on success. If an unexpected error aborts a run after `run-start`, the command best-effort appends `run-failed`. Completed group rows are flushed before the next group starts, so a missing terminal row indicates interruption after the last recorded group event. Report rows use safe IDs and counts only; they do not copy message text, media, credentials, DSNs, tokens, or session data. `group-unavailable` records the configured signed chat ID and a safe category for owner-inaccessible historical bindings such as missing input entity, invalid/private channel, unavailable public group, invalid chat/peer, or not-participant.
